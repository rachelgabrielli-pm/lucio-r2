...
def run_lucio(data: dict) -> dict:
    ...
    context = (
        ...
        f"MERCHANT PORTFOLIO (showing top 40 by repayment_pace_ratio):\n"
        f"{json.dumps([{'merchant_id':m['merchant_id'],'merchant_name':m['merchant_name'],'segment':m['segment'],'country':m['country'],'city':m.get('city',''),'financing_status':m['financing_status'],'repayment_pace_ratio':m.get('repayment_pace_ratio',0),'gross_sales_uplift_pct':m.get('gross_sales_uplift_pct',0),'gross_sales_pre_avg_monthly':m.get('gross_sales_pre_avg_monthly',0),'gross_sales_trend':m.get('gross_sales_trend',0),'avg_order_size':m.get('avg_order_size',0),'refund_rate':m.get('refund_rate',0),'customer_rating':m.get('customer_rating',0),'active_listings':m.get('active_listings',0),'avg_monthly_txn_count':m.get('avg_monthly_txn_count',0),'zero_sales_days_last_90d':m.get('zero_sales_days_last_90d',0),'disbursement_date':m.get('disbursement_date',''),'partner_revenue_share':m.get('partner_revenue_share',0),'total_prior_credits':m.get('total_prior_credits',0)} for m in sorted(merchants, key=lambda x: x.get('repayment_pace_ratio',0), reverse=True)[:40]], indent=2)}\n\n"
        "FIELD DEFINITIONS:\n"
